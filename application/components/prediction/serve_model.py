...
import numpy as np
from application.components.prediction import craft_utils
from application.components.prediction import imgproc
from application.components.prediction import file_utils
import json
import zipfile

from application.components.prediction.craft import CRAFT
...
parser.add_argument('--trained_model', default='models/craft_mlt_25k.pth', 
...
parser.add_argument('--refiner_model', 
    default='models/craft_refiner_CTW1500.pth', type=str, 
...
"""
low_text = 0.4
test_folder = "/Users/israel/tensorflow-fastapi-starter-pack/application/components/prediction/test"
mag_ratio = 1.5
canvas_size= 1280
show_time = True
cuda = False
refine = True
refiner_model = '/Users/israel/tensorflow-fastapi-starter-pack/application/components/prediction/models/craft_refiner_CTW1500.pth'
poly = False
link_threshold = 0.4
text_threshold = 0.7
trained_model = '/Users/israel/tensorflow-fastapi-starter-pack/application/components/prediction/models/craft_mlt_25k.pth'
...
def test_net(net, image, text_threshold, link_threshold, low_text, 
        cuda, poly, refine_net=None):
    t0 = time.time()

    # resize
    img_resized, target_ratio, size_heatmap = imgproc.resize_aspect_ratio(image,
        canvas_size, interpolation=cv2.INTER_LINEAR, mag_ratio=mag_ratio)
...
def predict(image_path):
    ...
    refine_net = None
    if refine:
        from application.components.prediction.refinenet import RefineNet
    ...
    api_dictionary = file_utils.saveResult(image_path, 
        image[:,:,::-1], bboxes, dirname=result_folder)

    response.append(api_dictionary)
